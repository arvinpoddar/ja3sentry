...
def extract_user_agent_string(request):
    """
...
def retrieve_http_req(sock, message_queues, sock_to_ja3, poller):
    ...
    if not init_request:
        ...
        return False
    if b"GET" not in init_request:
        _LOGGER.warning("Not a GET request")
        return False
    
    # data exists from the previous read
    try:
        _LOGGER.debug(init_request.decode("utf-8"))
        user_agent_string = extract_user_agent_string(init_request).decode("utf-8")
        browser_name = "Unknown"

        # it could extract the UA section of the header
        if user_agent_string != b"Unknown":
            # real quick check for any headless browser(s)
            found_headless = check_for_headless_browsers(user_agent_string)
            ...
            if found_headless is not None:
                ...
                parsed_ua = httpagentparser.detect(user_agent_string)
                browser = parsed_ua.get("browser", None)
                # the UA parser was able to successfully extract a browser
                if browser is not None:
                    browser_name = parsed_ua["browser"].get("name", None)

        # grab the ja3 associated with the socket
        ja3_record = sock_to_ja3[sock]
        browser_info = [ja3_record, browser_name, user_agent_string]
    ...
    try:
        ...
        if client_hello:
            ...
            if ja3_record is not None:
                ...
                _LOGGER.info("JA3: (%s,%s) :: %s", addr[0], addr[1], ja3_record)
            ...
            # we have output to send to the client
                ...
                # we've got nothing to send it
                    ...
                    try:
                        ...
                        _LOGGER.error("Client hung up before we could send back HTTP response")
